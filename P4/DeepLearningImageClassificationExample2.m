...
%
% Note: This example requires Neural Network Toolbox(TM), Statistics and
% Machine Learning Toolbox(TM), and Neural Network Toolbox(TM) Model _for
% AlexNet Network_ .
%
...
function DeepLearningImageClassificationExample2
%% Download Image Data
...
% imagesData=reshape(imagesData,636,128,128,1);
% imagesData = permute(imagesData,[2 3 4 1]);
% labels=permute(labels,[2 1]);
% labels=categorical(labels);
% imagesData = cat(3,imagesData,imagesData,imagesData);
% imagesData=imresize(imagesData,[227,227]);
% 
% featureLayer = 'fc7';
% imagesData = activations(net, imagesData, featureLayer, ...
%     'MiniBatchSize', 32, 'OutputAs', 'columns');
% 
% indexes = crossvalind('Kfold',size(imagesData,2),6);
% k=1;
% Xtrain = imagesData(:,indexes~=k);      
% Ytrain  = labels(indexes~=k,:);  
% Xtest  =   imagesData(:,indexes==k);  
% Ytest   = labels(indexes==k,:);
% 
% 
% 
...
% classifier
% size(trainingFeatures)
% estimatedLabels= predict(classifier,Xtest');
% %error
% error = mean(estimatedLabels==Ytest)
...
newImage = fullfile(rootFolder, '5', '7_5.tiff');
...
%
% [4] Donahue, Jeff, et al. "Decaf: A deep convolutional activation feature
